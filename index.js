...
import { lineSegment, SDFsquare } from "./lib/sdf/sdf.js";
...
const p1 = {x: -1 + 2 * Math.random(), y: -1 + 2 * Math.random()};
const p2 = {x: -1 + 2 * Math.random(), y: -1 + 2 * Math.random()};

const lp1 = {x: 0.5, y: 0.8};
const lp2 = {x: -0.2, y: 0};
...

{
  ...
  const loop = () => {
    ...
    for(let i = 0; i < 255; i++) {
      ...
      const square = SDFsquare({x: rx, y: ry}, 0.25);
...
/*{
  ...
  const loop = () => {
    for(let i = 0; i < canvas.width; i++) {
      ctx.fillStyle = `hsl(0deg 0% ${100 * Math.random()}%)`
